...
adminLogin.querySelector('form').addEventListener('submit',async event=>{
  ...
  error.textContent=''
  try{
    const response=await fetch(publicAsset('api/admin/login'),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:data.get('username'),password:data.get('password')})})
