...
adminLogin.querySelector('form').addEventListener('submit',async event=>{
  ...
  try{
    ...
    if(!response.ok){ const result=await response.json().catch(()=>({})); error.textContent=result.error||'Benutzername oder Passwort ist falsch.'; form.elements.password.value=''; form.elements.password.focus(); return }
...
admin.querySelector('.admin-upload').addEventListener('submit',async e=>{ e.preventDefault(); const form=e.currentTarget,files=[...form.elements.images.files],artist=form.elements.artist.value; if(!files.length)return; const data=new FormData(); data.append('artist',artist); files.forEach(file=>data.append('images',file)); adminStatus.textContent=`${files.length} Bilder werden hochgeladen …`; try{ const response=await fetch('/api/admin/uploads',{method:'POST',body:data}); const result=await response.json(); if(!response.ok)throw new Error(result.error||'Upload fehlgeschlagen.'); result.files.forEach((file,index)=>works.push({id:`w${Date.now()}-${index}`,artist,src:file.src})); normalizeArtistImages(artist); persist(); form.reset(); adminStatus.textContent=`${result.files.length} Bilder wurden dauerhaft auf dem Server gespeichert.` }catch(error){ adminStatus.textContent=error.message||'Upload fehlgeschlagen.' } })
renderAdmin()
